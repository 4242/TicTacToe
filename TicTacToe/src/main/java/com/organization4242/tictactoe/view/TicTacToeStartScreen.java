...
public class TicTacToeStartScreen extends Screen{
    ...
    private void drawField(){
        ...
        for (int i = 0; i < 9; i++) {
            ...
            fieldY = i / 3;
            for (int j = 0; j < 3; j++) {
                    switch (MainField.getInstance().getFields().get(i).get(j)) {
                        case O:
                            cl = Color.BLUE;
                            break;
                        case EMPTY:
                    ...
                    g.drawRect(10 + fieldX * 190 + ((j + 1) - (j + 1) % 3) * 60, 10 + fieldY * 190 + j * 60,
                            50, 50, cl);
            }
        }
    }
        ...
        for (int i = 0; i < len; i++){
            ...
            if (event.getType() == Input.TouchEvent.TOUCH_UP){
                ...
                if ( (touchedMainFieldX < 3) && (touchedFieldY < 3) ) {
                    firePropertyChange(Controller.VIEW_UPDATED, 0,
